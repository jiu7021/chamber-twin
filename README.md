...
공개 실측 데이터 96 웨이퍼를 분석하고, 물리 모델을 세우고, PLC/SCADA 연동까지 만들었다.
...
`data/` 는 저장소에 포함하지 않는다. `scripts/download.sh` 로 내려받는다.
OES 스펙트럼(약 8 GB)은 이 프로젝트에서 사용하지 않는다.
...
그래서 물리 법칙으로 그 값을 계산해내는 가상 챔버를 만들었고, 진짜 PLC를 붙여 장비 없이 제어·알람까지 시험할 수 있게 했다.
